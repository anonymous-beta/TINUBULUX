...
def confirm_scope(args, logger):
    """Require explicit confirmation unless --yes is passed."""
    if args.yes:
        return True
    logger.warning("This tool sends real traffic. Confirm authorization:")
    logger.warning(f"  Target: {args.target}:{args.port} | Method: {args.method.upper()}")
    try:
        answer = input("Do you have written authorization for this target? [y/N]: ")
    except EOFError:
        answer = ""
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python3 tinubulux.py -t 192.168.1.1 -p 80 -m http -c 500 -y
  python3 tinubulux.py -t target.com -m udp --duration 300 -r 1000 -y
  python3 tinubulux.py -t 10.0.0.5 -m syn -c 1000 --no-ui -y
        """
    )
    ...
    parser.add_argument('-r', '--rate', type=int, default=0,
                        help='Packets/sec PER WORKER (0 = unlimited, default: 0)')
    ...
    if not confirm_scope(args, logger):
        logger.error("Aborted: no authorization confirmed.")
        sys.exit(1)

    engine = AttackEngine(args, config, logger)

    # Proper signal wiring — no globals() fragility
    def on_sigint(sig, frame):
        logger.warning("TINUBULUX shutting down gracefully...")
        engine.stop()
        sys.exit(0)
